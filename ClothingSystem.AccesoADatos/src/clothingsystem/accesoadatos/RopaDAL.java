...
public class RopaDAL { // Clase para poder realizar consulta de Insertar, modificar, eliminar, obtener datos de la tabla Rol
    ...
    static String obtenerCampos() {
        return "r.Id, r.IdMarca, r.CodigoBarra, r.Nombre, r.PrecioCompra, r.PrecioVenta, r.Existencia, r.Estatus, r.Talla, r.Color, r.Estilo, r.Descripcion, r.TopoTela";
    }
    ...
    public static int crear(Ropa pRopa) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pRopa.getNombre()); // Agregar el parametro a la consulta donde estan el simbolo ? #1  
    ...
    public static int modificar(Ropa pRopa) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pRopa.getNombre()); // Agregar el parametro a la consulta donde estan el simbolo ? #1  
                ps.setInt(2, pRopa.getId()); // Agregar el parametro a la consulta donde estan el simbolo ? #2  
    ...
    static int asignarDatosResultSet(Ropa pRopa, ResultSet pResultSet, int pIndex) throws Exception {
        ...
        pIndex++;
        pRopa.setNombre(pResultSet.getString(pIndex)); // index 2
        return pIndex;
    }
